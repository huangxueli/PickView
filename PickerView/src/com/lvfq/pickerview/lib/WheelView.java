...
public class WheelView extends View {
	...
	// 条目间距倍数
	static final float dividerHeight = 0.3F;
	boolean isLoop;

	// 第一条线Y坐标值
	float firstLineY;  	// Y值较小(上面那条)
	// 第二条线Y坐标
	float secondLineY;	// Y值较大(下面那条)
	// 中间Y坐标
	...
	private static final int VELOCITYFLING = 5;// 修改这个值可以改变滑行速度
	...
	private float lablePadingRightOffset = 0;// Label右边距偏移值
	private float lableYOffset = 0;// LabelY轴偏移值
	private float centerContentYOffset = 0;// 中间内容Y轴偏移值
	private float contentXOffset = 0;// 内容X轴偏移值
	...
	public WheelView(Context context, AttributeSet attrs) {
		...
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.wheelview, 0, 0);
		mGravity = ta.getInt(R.styleable.wheelview_gravity, Gravity.CENTER);
		textColorOut = ta.getColor(R.styleable.wheelview_textColorOut, textColorOut);
		textColorCenter = ta.getColor(R.styleable.wheelview_textColorCenter, textColorCenter);
		dividerColor = ta.getColor(R.styleable.wheelview_dividerColor, dividerColor);
		textSize = ta.getDimensionPixelOffset(R.styleable.wheelview_textSize, textSize);
		lablePadingRightOffset = ta.getFloat(R.styleable.wheelview_lablePadingRightOffset, lablePadingRightOffset);
		lableYOffset = ta.getFloat(R.styleable.wheelview_labelYOffset, lableYOffset);
	...
	protected void onDraw(Canvas canvas) {
		...
		// 单位的Label
		if (label != null) {
			float offset = getTextHeight(paintCenterText, label)/2.0F;
			int drawRightContentStart = measuredWidth - getTextWidth(paintCenterText,label);
//			int centercontentStart = (int) ((measuredWidth - (maxTextWidth + lablePadingLeftOffset + getTextWidth(paintCenterText, label))) * 0.5);
//			int centercontentStart = drawCenterContentStart;
//			int drawLabelStart = (int) (drawRightContentStart + maxTextWidth + lablePadingLeftOffset);
			canvas.drawText(label, drawRightContentStart- lablePadingRightOffset, measuredHeight/2 + offset + lableYOffset, paintCenterText);
		}
		counter = 0;
		while (counter < itemsVisible) {
			canvas.save();
			...
			if (angle >= 90F || angle <= -90F) {
				...
				float translateY = (float) (radius - cos - (sin * maxTextHeight) / 2D); 
				
				float offset = getTextHeight(paintCenterText, contentText)/2.0F;
				if (translateY >= firstLineY && maxTextHeight + translateY <= secondLineY) {
					// 中间条目
					
//					float offset1 = getTextHeight(paintCenterText, "\u661F\u671F")/2.0F;
					canvas.drawText(contentText, drawCenterContentStart + contentXOffset, measuredHeight/2 + offset + centerContentYOffset, paintCenterText);
					...
					canvas.translate(0.0F, translateY);// 设置起始坐标为（0.0， translateY）
					canvas.scale(1.0F, (float) Math.sin(radian));
					// 条目经过第一条线
					canvas.save();
					canvas.clipRect(0, 0, measuredWidth, firstLineY - translateY);
					canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT);
					canvas.drawText(contentText, drawOutContentStart + contentXOffset, maxTextHeight, paintOuterText);
					canvas.restore();
					canvas.save();
					canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int) (itemHeight));
					canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
					canvas.drawText(contentText, drawCenterContentStart + contentXOffset, maxTextHeight - offset, paintCenterText);
					canvas.restore();
	...
	private void measureTextWidthHeight() {
		...
		for (int i = 0; i < adapter.getItemsCount(); i++) {
			...
			paintCenterText.getTextBounds("\u661F\u671F", 0, 2, rect); // 星期    //"\u661F\u671F"
			int textHeight = rect.height();
			if (textHeight > maxTextHeight) {
				maxTextHeight = textHeight;
			}
		}
		itemHeight = dividerHeight * maxTextHeight * 2 + maxTextHeight;
	}
...
//			int labelwidth = getTextWidth(paintCenterText, label);
//			drawCenterContentStart = (int) ((measuredWidth - (contentwidth + lablePadingLeftOffset + labelwidth)) * 0.5);
...
//			drawOutContentStart = (int) ((measuredWidth - (rect.width() + lablePadingLeftOffset + getTextWidth(paintCenterText, label))) * 0.5);
			break;
		case Gravity.LEFT:
	...
	public void setCustomTextSize(boolean custom) {
		customTextSize = custom;
	}
	
	public void setLableYOffset(float y){
		lableYOffset = y;
	}
	
	public void setLablePadingRight(float padingright){
		lablePadingRightOffset = padingright;
	}
	
	public void setCenterContentYOffset(float y){
		centerContentYOffset = y;
	}
	
	public void setContentXOffset(float offset){
		contentXOffset = offset;
	}
}